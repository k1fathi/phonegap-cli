 ...
 * @return {boolean} true if help flag detected
 * 
 */
function detectHelp(argv) {
    var help = detectHelp.indexOf('help');
...
function CLICore(argv, callback) {
    ...
    if (parsed.h || parsed.help || argv.indexOf('help') > -1) {
        help(parsed._, callback);
    } else if (parsed._[0] === 'serve' || parsed._[0] === 'version') {
        localcommand = require('.' + path.join( '/', 'clobber', parsed._[0]));
        // execute phonegap implementation of the subcommand
        localcommand(argv.slice(2), callback);
    } else if (parsed.v || parsed.version) {
        localcommand = require('.' + path.join( '/', 'clobber', 'version'));
        localcommand(argv, callback);
    } else if (parsed._[0] === 'local') {
        // splice out 'local'
        argv.splice(argv.indexOf('local'), 1);
        // recurse with new command
        CLICore(argv, callback);
